...
def featureExtractCRF(token):
    features = [int(token.isupper()),int(token.upper() in st_abbr_val),int(token[0].isupper()),int(token.islower()),int(num_abbr.findall(token)!=None),int(ampm.findall(token)!=None),len(times_.findall(token)),int(timezone.findall(token)!=None), int(token.upper() in directions),\
                int(punctuations in token),int(token.isdigit()),len(DigitLetter.findall(token)),int(email.findall(token)!=None),len(token),int(token[0].isalpha())]
    return features
...
def addressBigrams1(k,addr,addr_label):
        ...
        #print 'start:',' '.join(addr[:k+2])

        addr_label[' '.join(addr[-k-2:])] = 2
        #print 'end:',' '.join(addr[-k-2:])
...
def FeatureTokenCRF(b,k,index,tokens,entities,x,y,addressDict):
    bigrams = []
    dict_Bigrams1 = {}
    count_bigram = collections.defaultdict(int)
    
    for i in xrange(b,len(tokens)-k):
        bigram = tokens[i-b:i+k+1]
        bigram_ = ' '.join(bigram)
        count_bigram[bigram_]+=1
        if bigram_ in addressDict:
            y.append(addressDict[bigram_])
        ...
        if bigram_ not in dict_Bigrams1:
            features =featureExtractCRF(bigram[b])#central word
            dict_Bigrams1[bigram_]=np.array(features)
...
def createCRF(k,lis,entities,address_csv=''):
    bigrams = []
    count_bigrams = []
    ...
    for i,l in enumerate(lis):
        ...
        sentences = [sentence for sentence in l.split('.') if sentence]
        for sentence in sentences:
            ...
            if i in addr_label:
                address_dict = addr_label[i]
            bigrams_,count_bigram=(FeatureTokenCRF(k,k,i,tokens,entities,x,y,address_dict))
            bigrams+=bigrams_
            
            if y:
                outX.append(x)
                Y.append(y)
                Types.append(len(set(y)))

    #print count_unigram
    count_bigrams.append(count_bigram)
    
    return bigrams,count_bigrams,outX,word_dist_list,time_dist_list,Y,Types



def main():
    k=1 #context window
    #for labeled university wire
    #textlist,entities = read_taggedFiles('University Wire_output/*.txt',k)
    #bigramList,count_bigrams,outX,word_dist_list,time_dist_list,Y = createBigramList(k,textlist,entities,'UW_address_window'+str(k)+'.csv')

    import cPickle

    #for labeled Washingtonpost
    textlist,entities = read_taggedFiles('WP_output2/*.txt',k)
    #bigrams,count_bigrams,outX,Y = createBigramList(k,textlist,entities,'WP_address_window1.csv')
    #print set(Y)
    #cPickle.dump(outX,open('WP_trainX_window_'+str(k)+'.p','wb'))
    #cPickle.dump(Y,open('WP_trainY_window_'+str(k)+'.p','wb'))

    bigramList,count_bigrams,outX,word_dist_list,time_dist_list,Y,Types = createCRF(k,textlist,entities,'WP_address_window1.csv')
    cPickle.dump(outX,open('WP_trainX_CRF_window_'+str(k)+'.p','wb'))
    cPickle.dump(Y,open('WP_trainY_CRF_window_'+str(k)+'.p','wb'))
    cPickle.dump(Types,open('WP_types_CRF_window_'+str(k)+'.p','wb'))

    #for unlabled data set
    #textlist,entities = read_taggedFiles('WP_1516_output/*.txt',k)
    #bigramList,count_bigrams,outX,word_dist_list,time_dist_list,Y = createBigramList(k,textlist,entities,'')


    #cPickle.dump(outX,open('WP_1516_trainX_window_'+str(k)+'.p','wb'))
    
    #cPickle.dump(outX,open('UW_trainX_window_'+str(k)+'.p','wb'))
    #cPickle.dump(Y,open('UW_trainY_window_'+str(k)+'.p','wb'))
    #cPickle.dump(bigramList,open('UW_bigramList_window_'+str(k)+'.p','wb'))
